...
/**
 * Utility class for extracting ZIP and JAR archives for MuleGuard validation.
...
public class ArchiveExtractor {
     ...
     * Pattern: ${baseTempDir}/muleguard-sessions/${sessionId}/
     * 
     * @param sessionId Unique session identifier
     * @param baseTempDir Base temporary directory
    ...
    private static Path createTempDirectory(String sessionId, String baseTempDir) throws IOException {
        Path tempSessionsDir = Paths.get(baseTempDir, "muleguard-sessions");
    ...
    public static void cleanupSession(String sessionId, String baseTempDir) {
        try {
            Path sessionDir = Paths.get(baseTempDir, "muleguard-sessions", sessionId);
            if (Files.exists(sessionDir)) {
