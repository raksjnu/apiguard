 ...
 * - Enforces expiry date
 * - Limits number of executions
 * - Stores usage data in user's home directory
 */
public class TrialLicenseManager {
    
    // ========== TRIAL MODE CONFIGURATION ==========
    // Set this to false to disable trial restrictions (full version)
    // Set this to true to enable trial restrictions (trial version)
    private static final boolean TRIAL_MODE = false;
    // ===============================================
    
    private static final String LICENSE_FILE = ".muleguard-trial.dat";
    private static final String EXPIRY_DATE = "2026-02-01"; // Set your expiry date here
     ...
     * @return true if trial is valid, false otherwise
     * @throws LicenseException if trial has expired or limit exceeded
     */
    public boolean validateTrial() throws LicenseException {
        // If trial mode is disabled, always return true (full version)
        if (!TRIAL_MODE) {
            return true;
        }
