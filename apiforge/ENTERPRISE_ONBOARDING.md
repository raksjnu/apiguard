...
| `ApiForgeWeb.java` | Powers the reactive GUI and Toolkit API endpoints. |
...
**Output:** `target/apiforge-1.0.0.jar` (~15 MB)
...
|----------|---------|------|
| `apiforge-1.0.0-jar-with-raks.jar` | Executable application | ~15 MB |
| `config.yaml` | Configuration template (optional) | ~5 KB |
| `start-apiforge-gui.bat` / `.sh` | GUI launcher scripts | ~1 KB |
...
unzip apiforge-1.0.0-dist.zip

# Run GUI mode
./start-apiforge-gui.sh

# Run CLI mode
java -jar apiforge-1.0.0-jar-with-raks.jar --output results.json
```
...
WORKDIR /app
COPY apiforge-1.0.0-jar-with-raks.jar .
CMD ["java", "-jar", "apiforge-1.0.0-jar-with-raks.jar"]
```
...
nohup java -jar apiforge-1.0.0-jar-with-raks.jar --gui &
