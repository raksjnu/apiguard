...
public class GitHubProvider implements GitProvider {
    ...
    public GitHubProvider(String userToken) {
        String url = config.getProperty("github.url");
        ...
        if (userToken != null && !userToken.isBlank()) {
            this.token = userToken;
             ...
             this.token = config.getProperty("github.token");
        }
    }
    ...
    public List<String> listBranches(String repoName) throws Exception {
        String fullPath = cleanRepoName(repoName);
        String apiUrl = baseUrl + "/repos/" + fullPath + "/branches?per_page=100";
    ...
    public List<Map<String, String>> listRepositories(String groupName) throws Exception {
        HttpClient client = HttpClient.newHttpClient();
        List<Map<String, String>> allRepos = new ArrayList<>();
        
        String url;
        if (groupName == null || groupName.isBlank()) {
            url = baseUrl + "/user/repos?per_page=100&sort=updated";
        } else {
            url = baseUrl + "/orgs/" + groupName + "/repos?per_page=100&sort=updated";
        }
        
        HttpResponse<String> response = sendWithAuthFallback(client, url);
        
        if (response.statusCode() == 404 && groupName != null) {
            response = sendWithAuthFallback(client, baseUrl + "/users/" + groupName + "/repos?per_page=100&sort=updated");
        }
        ...
        if (response.statusCode() == 200) {
            ...
            if (root.isArray()) {
                for (JsonNode node : root) {
                    if (node.has("full_name") && node.has("clone_url")) {
                        Map<String, String> repo = new HashMap<>();
                        repo.put("name", node.get("full_name").asText());
                        repo.put("cloneUrl", node.get("clone_url").asText());
                        allRepos.add(repo);
                    }
                }
            }
